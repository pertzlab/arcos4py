    ...
    **kwargs: The **kwargs from the calling function.
    ...
    for old_param, new_param in param_mapping.items():
        if old_param in updated_kwargs:
            ...
            warnings.warn(f"The '{old_param}' parameter is deprecated and will be removed in a future version. Please use '{new_param}' instead.", DeprecationWarning, stacklevel=3)
            if new_param in updated_kwargs:
                # If both new and old parameters are provided, overwrite the new with the old
                warnings.warn(f"Both '{old_param}' and '{new_param}' are provided. '{old_param}' will take precedence over '{new_param}'. Consider using only '{new_param}' in future.", DeprecationWarning, stacklevel=3)
